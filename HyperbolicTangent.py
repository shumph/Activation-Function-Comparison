...
def ddx_tanh(value):
    return 1.-np.tanh(value)**2      #d/dx(tanhx) = 1 - tanh^2(x)
...
def forward_propagation(input,weight_one,bias_one,weight_two,bias_two):
    ...
    active_layer_one = tanh(layer_one) 
...
def backward_propagation(input, target, active_layer_one, active_layer_two, weight_two, layer_one, rows):
    ...
    ddx_layer_one = weight_two.T.dot(ddx_layer_two)*ddx_tanh(layer_one) 
...
def gradient_descent_tanh(input, target, hyperparamater, iterations, w, h):
...
def main():
    ...
    weight_one, bias_one, weight_two, bias_two, accuracy_list, image_list = gradient_descent_tanh(input_train, target_train, 0.15, 50, w, h)
    print(accuracy_list)
    #print(image_list)
#main()