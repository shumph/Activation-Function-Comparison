...
def relu(value):
    return np.maximum(value,0)  # d/dx(0) = 0, d/dx(x) = 1
...
def forward_propagation(input,weight_one,bias_one,weight_two,bias_two):
    ...
    active_layer_one = relu(layer_one) 
...
def backward_propagation(input, target, active_layer_one, active_layer_two, weight_two, layer_one, rows):
    ...
    ddx_layer_one = weight_two.T.dot(ddx_layer_two)*ddx_relu(layer_one) 
...
def gradient_descent_relu(input, target, hyperparamater, iterations, w, h):
...
def main():
    ...
    weight_one, bias_one, weight_two, bias_two, accuracy_list, image_list = gradient_descent_relu(input_train, target_train, 0.15, 200, w, h)
    print(accuracy_list)
    #print(image_list)
#main()